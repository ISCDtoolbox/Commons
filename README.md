...
on Debian systems such as Ubuntu, you can use the `apt` software as follows.
First type in a command prompt
```
sudo apt-add-repository main
```
in order to let your system access the main repository that contain the
canonical-supported free and open-source softwares such as `gcc`, `make`, or
`cmake` (normally this should be already active by default). Similarly, type in
a command prompt
```
sudo apt-add-repository universe
```
which is the repository that contains community-maintained free and open-source
softwares such as `clang-3.8`. We recall that the `sudo` word implies that you
will run a command using the administrator rights. If you do not have the
password for this, you may have to contact your system administrator in order to
install it properly. Then, you may also have to update the repositories which
can be done by typing in a command prompt
```
sudo apt-get update
```
For example, we had to do it for finding `git` in the main repository of one of
our test system (Ubuntu 14.04.5). You can search for a specific package name by
...
* `libc6-dev` (version 2.23) package which contains the GNU C Library
...
to search for libraries in a Linux system. First of all, in the case you want to
...
*libc.so*, *libm.so*, *libpthread.so*, and the optional ones
*libgomp.so*/*libomp.so* or *libiomp5.so*) or simply a file (such as their
associated headers like *stdlib.h*, *math.h*, *pthread.h*, *omp.h*) by typing in
a command prompt
```
...
remark below)
* `make` (version >= 3.81)
* `cmake` (version >= 3.11.4 (>= 2.8 is mandatory) and version >= 3.12 is
...
familiar with Mac OS). For example, you can use the [Homebrew](https://brew.sh/)
software as follows. First, you may have to update the local repository
containing the list of available packages. This can be done by typing in a
command prompt
```
brew update
```
...
default, the C-standard library *libc.dylib*, the Math library *libm.dylib*, and
the Thread library *libpthread.dylib* should already be installed by default on
...
[Homebrew](https://brew.sh/). We now detail how to search for libraries in a
Mac OS system. First of all, in the case you want to use multi-threading, you
can obtain the total number of cpu of your system by typing in a command prompt
```
sysctl hw
```
You can also use the `locate` command to quickly search for a library but you
...
```
Then, you can search for a library (such as the previous required ones
*libc.dylib*, *libm.dylib*, *libpthread.dylib*, and the optional ones
*libgomp.dylib* / *libomp.dylib* or *libiomp5.dylib*) or simply a file (such as
their associated headers like *stdlib.h*, *math.h*, *pthread.h*, *omp.h*) by
typing in a command prompt
```
locate NameOfTheLibraryOrTheFileYouWantToSearch
```
...
`ls` commands, cf. the end of next installation path section if you are not
...
download the files manually from github in a *\*.zip* archive format, just go to
the upper right part of the current
[project page](https://github.com/ISCDtoolbox/Commons), click on the
...
install locally the library (respectively the associated public header files) in
a *lib/* (resp. *include/*) subdirectory of the *Commons/* directory, just
